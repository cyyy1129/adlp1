...
function calculate(history, overrides = {}) {
  return calculateForecast({
    ...
    weather: overrides.weather ?? { availability: 'available', condition: 'clear', summary: 'Verified clear condition.', source: 'test' },
...
const caseA = calculate(manyHistory, { events: [{ name: 'Food fair', distance_km: 2, starts_at: null, source: 'test' }] });
...
assert.deepEqual(caseA, calculate(manyHistory, { events: [{ name: 'Food fair', distance_km: 2, starts_at: null, source: 'test' }] }), 'Same input must be deterministic');
...
const caseC = calculate(manyHistory, { weather: { availability: 'unavailable', condition: null, summary: 'Unavailable', source: null } });
...
const caseF = calculate([session({ sold: 50 }), session({ sold: 70 }), session({ sold: 80 })], {
  weather: { availability: 'available', condition: 'other', summary: 'Other', source: 'test' },
});
...
const friday = calculate([session({ sold: 60, date: '2026-09-11' })], {
  date: '2026-09-11', weather: { availability: 'available', condition: 'other', summary: 'Other', source: 'test' },
});
const saturday = calculate([session({ sold: 60, date: '2026-09-11' })], {
  date: '2026-09-12', weather: { availability: 'available', condition: 'other', summary: 'Other', source: 'test' },
});
